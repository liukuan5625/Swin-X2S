...
inputs: torch.Tensor,
    ...
    **kwargs: Any,
    ...
    if sptial_dims == 2:
        return double_sliding_window_inference_2d(
            inputs,
            roi_size,
            sw_batch_size,
            predictor,
        ...
        return double_sliding_window_inference_3d(
            inputs,
            roi_size,
            sw_batch_size,
            predictor,
...
def double_sliding_window_inference_2d(
    inputs: torch.Tensor,
    roi_size: Union[Sequence[int], int],
    sw_batch_size: int,
    predictor: Callable[..., Union[torch.Tensor, Sequence[torch.Tensor], Dict[Any, torch.Tensor]]],
    ...
    **kwargs: Any,
    ...
    num_spatial_dims = len(inputs.shape) - 3
    if overlap < 0 or overlap >= 1:
        raise ValueError("overlap must be >= 0 and < 1.")

    # determine image spatial size and batch size
    # Note: all input images must have the same image size and batch size
    batch_size, _, _, *image_size_ = inputs.shape
    ...
    image_size = tuple(max(image_size_[i], roi_size[i+1]) for i in range(num_spatial_dims))
    pad_size = []
    for k in range(len(inputs.shape) - 2, 1, -1):
        diff = max(roi_size[k - 2] - inputs.shape[k + 1], 0)
        half = diff // 2
    ...
    scan_interval = _get_scan_interval(image_size, roi_size[1:], num_spatial_dims, [overlap]*num_spatial_dims)

    # Store all slices in list
    slices = dense_patch_slices(image_size, roi_size[1:], scan_interval)
    num_win = len(slices)  # number of windows per image
    total_slices = num_win * batch_size  # total number of windows

    # Create window-level importance map
    valid_patch_size = get_valid_patch_size(image_size, roi_size[1:])
    if valid_patch_size == roi_size[1:] and (roi_weight_map is not None):
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        window_data_list = [window_data[:, 0], window_data[:, 1]]
        if is_cross:
            seg_prob_out_1, seg_prob_out_2 = predictor(
                window_data_list[0], window_data_list[1], *args, **kwargs
            )  # batched patch segmentation
        else:
            seg_prob_out = predictor(
                torch.cat(window_data_list, dim=1), *args, **kwargs
            )  # batched patch segmentation
            seg_prob_out_1 = seg_prob_out_2 = seg_prob_out
        ...
        for ss in range(len(seg_prob_tuple_1)):
            ...
            zoom_scale = []
            image_size = [image_size[0], image_size[0], image_size[1]]
            window_data_shape = list(window_data.shape[3:4]) + list(window_data.shape[3:])
            for axis, (img_s_i, out_w_i, in_w_i) in enumerate(
                zip(image_size, seg_prob_1.shape[2:], window_data_shape)
            ):
...
def double_sliding_window_inference_3d(
        inputs: torch.Tensor,
        roi_size: Union[Sequence[int], int],
        sw_batch_size: int,
        predictor: Callable[..., Union[torch.Tensor, Sequence[torch.Tensor], Dict[Any, torch.Tensor]]],
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        for ss in range(len(seg_prob_tuple_1)):
            ...
            zoom_scale = []
            for axis, (img_s_i, out_w_i, in_w_i) in enumerate(
                    zip(image_size, seg_prob_1.shape[2:], window_data.shape[3:])
            ):
                _scale = out_w_i / float(in_w_i)
                if not (img_s_i * _scale).is_integer():
                    warnings.warn(
