...
def model_visualiztion(path_name, sample_name, out_channels, num_cor_slice, num_sag_slice):
    ...
    fig, axs = plt.subplots(2, 11, figsize=(11, 2.5), dpi=3000)
    ...
    for i, model in enumerate(model_list):
        ...
        if model in ["tlpredictor", "onedconcat"]:
            ...
            seg_onehot = one_hot(torch.from_numpy(seg_ct[:, :, 16: 144].copy()).unsqueeze(0), num_classes=out_channels,
                                 dim=0)
            pred_onehot = one_hot(torch.from_numpy(pred_ct[:, :, 16: 144].copy()).unsqueeze(0),
                                  num_classes=out_channels, dim=0)
        else:
        ...
        '''axs[0, 10].imshow(raw_ct[:, num_cor_slice].T, cmap="gray")
        axs[0, 10].imshow(masked_seg[:, num_cor_slice].T, cmap=custom_cmap_pal, vmin=0, vmax=out_channels)'''
        axs[0, 10].imshow(label_cor_img)
        axs[0, 10].axis("off")
        '''axs[1, 10].imshow(raw_ct[num_sag_slice].T, cmap="gray")
        axs[1, 10].imshow(masked_seg[num_sag_slice].T, cmap=custom_cmap_pal, vmin=0, vmax=out_channels)'''
        axs[1, 10].imshow(label_sag_img)
        axs[1, 10].axis("off")
...
if __name__ == "__main__":
    # CTPelvic1K
    path_name = "./vis_result/models/CTPelvic1K"
    sample_name = "1.3.6.1.4.1.9328.50.4.0243"
    out_channels = 4
    model_visualiztion(path_name, sample_name, out_channels, num_cor_slice=66, num_sag_slice=35)

    # CTSpine1K
    path_name = "./vis_result/models/CTSpine1K"
    sample_name = "1.3.6.1.4.1.9328.50.4.0646"
    out_channels = 26
    model_visualiztion(path_name, sample_name, out_channels, num_cor_slice=45, num_sag_slice=64)

    # TotalAll
    path_name = "./vis_result/models/Totalall"
    sample_name = "s0561"
    out_channels = 55
    model_visualiztion(path_name, sample_name, out_channels, num_cor_slice=48, num_sag_slice=64)
