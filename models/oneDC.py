...
class OneDConcat(nn.Module):
    ...
    def __init__(
        ...
        self.fully_connected_bottlneck = nn.Sequential(
            nn.Linear(
                in_features=self._calculate_1d_vec_channels(
                    input_image_size, encoder_kernel_size, encoder_strides, encoder_out_channels
                ),
        ...
        self.decoder = nn.Sequential(*self._decoder_layers(
            decoder_in_channels, decoder_out_channels, decoder_strides, decoder_kernel_size, activation, norm, dropout,
            dropout_rate, bias
        ))
    ...
    def _calculate_1d_vec_channels(self, input_image_size, encoder_kernel_size, encoder_strides, encoder_out_channels) -> int:
        ...
        self.enc_conv_out_shape = input_image_size
        for encoder_stride in encoder_strides:
            self.enc_conv_out_shape = calculate_out_shape(
                in_shape=self.enc_conv_out_shape,
                kernel_size=encoder_kernel_size,
                stride=encoder_stride,
                padding=same_padding(encoder_kernel_size, dilation=1),
            )

        return int(encoder_out_channels[-1] * np.prod(self.enc_conv_out_shape)) * 2
