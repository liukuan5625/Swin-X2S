...
files_path = "./visualization/vis_result/morph/totalall_rib"
morph_name = ["clDice ↑",
              "nlDice ↑",
              "LSCDError ↓"]
info_lists = []
    ...
    for key, value in morph_dic.items():
        ...
        for k, v in value.items():
            for k_, v_ in v.items():
                info.append(v_)
            info_list.append(np.array(info).reshape([-1, 4]).T)

    print(len(info_list))
    if len(info_list) == 0:
        info_lists.append(np.array([[np.nan, np.nan, np.nan, np.nan]]).T)
    else:
...
for i in range(3):
    ...
    for info in info_lists:
        if info.shape[1] != 1:
            nan_mask = np.isnan(info).any(axis=0)
            info = info[:, ~nan_mask]
    ...
    plt.ylim(min(show_points) - 0.05, max(show_points) + 0.05)
    if i == 2:
