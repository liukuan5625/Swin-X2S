...
files_path = "./visualization/vis_result/morph/totalall_spine"
morph_name = ["spl",
              "spa",
              "avbh",
              "pvbh",
    ...
    for key, value in morph_dic.items():
        ...
        for k, v in value.items():
            for k_, v_ in v.items():
                info.append(v_)
            info_list.append(np.array(info).reshape([-1, 8]).T)
...
for i in range(7):
    ...
    plt.ylim(min(show_points) - 0.25, max(show_points) + 0.25)
    if i == 1:
        plt.ylabel(rf"${morph_name[i]}$" + "° ↓", fontsize=10, labelpad=0)
