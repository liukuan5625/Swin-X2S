...
files_path = "./visualization/vis_result/morph/totalall_pelvic"
morph_name = ["ASIS_L",
              "ASIS_R",
              "PT_L",
              "PT_R",
...
for file in os.listdir(files_path):
    ...
    info_lists.append(np.array(info_list).T)
...
for i in range(8):
    ...
    # Model
    scatter = plt.scatter(dice_points, show_points, c=np.linspace(0, 1, len(dice_points)), s=100, cmap='viridis',
                          alpha=0.8)
    ...
    # plt.show()
    plt.savefig(os.path.join(files_path, str(i)+morph_name[i]+".svg"), transparent=True)
